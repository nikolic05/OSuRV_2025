# Finger Gate (UART fingerprint + PIN unlock)

Mali C projekat koji radi kao **kontrola pristupa**: u pozadini stalno skenira prst preko **UART fingerprint modula (Waveshare/R503-kompatibilan protokol)**, a zatim kroz CLI komande (uz PIN) omogućava:
...
Ako si ranije uploadovao fajlove u chat, moguće je da su neki *stari uploadovi istekli* u sistemu. Za ovaj README sam se oslonio na fajlove koji su trenutno prisutni u projektu (npr. `main.c`, `config.h`, `Makefile`, itd.). Ako ti fali neki deo projekta (npr. pravi `gate.c`), samo ga uploaduj ponovo.

---

## License

Dodaj licencu po potrebi (MIT/BSD/GPL) ili ostavi “internal / school project”.
